...
export const updateFeedSource = async (
	id: string,
	data: { title: string; url: string; active?: boolean },
) => {
	return await db.feedSource.update({ where: { id }, data });
};
export const createFeedSource = async ({
	title,
	url,
}: {
	title: string;
...
export const createFeedDelivery = async ({
	hour,
	active,
	activeDays,
	timeZone,
	...
	return await promiseMap(
		...
		async (delivery) => {
			...
			const [items, updatedDelivery] = await db.$transaction([
				db.feedItem.updateMany({
					where: { delivered: false },
					data: { delivered: true },
				}),
				db.feedDelivery.update({
...
export const syncFeed = async (feedId: string) => {
	try {
		...
		const { updates, creates } = remoteFeed.items.reduce(
			(agg, remoteItem) => {
