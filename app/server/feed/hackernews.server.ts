...
export const getHackerNewsItem = async (id: number): Promise<FeedItem | null> => {
	try {
		...
		return {
			title: item.title,
			author: item.by,
			content: `Url: ${item.url} Points: ${item.score} Comments: ${
				(item.kids || []).length
			}`,
			url: item.url,
			commentsUrl: (item.kids || []).length
...
export const getFrontPageItems = async (from = 0, count = 100): Promise<FeedItem[]> => {
	...
	const items = await promiseMap(ids.slice(from, count), getHackerNewsItem, {
		concurrency: 25,
	});
